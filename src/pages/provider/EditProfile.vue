...
export default {
  ...
  mounted() {
    ...
    axios.request({
      ...
      method: "GET",
      url: baseurl.url + 'v1/service/all'
    }).then(response => {
    ...
    axios.request({
      ...
      method: "GET",
      url: baseurl.url + 'v1/provider/profile'
    }).then(response => {
      let $profile = response.data;
      if ($profile.status == 1) {
        this.val_state_name = $profile.state_name;
  ...
  methods: {
    ...
    async updateProfile() {
      ...
      let token = JSON.parse(localStorage.getItem('token'))
      let status = '';
      await axios.post(baseurl.url + 'v1/provider/editprofile', {
    ...
    CityList(state) {
      ...
      for (let option in options) {
        this.city_options += options[option].outerHTML;
      }
    }
  }
}
